...
for ((i=0; i<3; i++)); do
    ...
    if script_output=$(./run.sh "$trip_duration" "$miles_traveled" "$receipts_amount" 2>/dev/null); then
        ...
        if [[ $output =~ ^-?[0-9]+\.?[0-9]*$ ]]; then
            ...
            error=$(py -c "print(abs($actual - $expected))")
            ...
            if py -c "exit(0) if abs($error) < 0.01 else exit(1)"; then
                exact_matches=$((exact_matches + 1))
            fi
            
            # Check for close match (within $1.00)
            if py -c "exit(0) if abs($error) < 1.0 else exit(1)"; then
                close_matches=$((close_matches + 1))
            fi
            
            # Update total error
            total_error=$(py -c "print($total_error + $error)")
            
            # Track maximum error
            if py -c "exit(0) if $error > $max_error else exit(1)"; then
                max_error="$error"
...
if [ $successful_runs -eq 0 ]; then
    ...
    avg_error=$(py -c "print(round($total_error / $successful_runs, 2))")
    
    # Calculate percentages
    exact_pct=$(py -c "print(round($exact_matches * 100 / $successful_runs, 1))")
    close_pct=$(py -c "print(round($close_matches * 100 / $successful_runs, 1))")
    ...
    score=$(py -c "print(round($avg_error * 100 + ($num_cases - $exact_matches) * 0.1, 2))")
